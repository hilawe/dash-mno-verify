...
- The gateway. It learns the platform account the challenge was minted for (the B1 relay fix binds that account into the signal hash the proof commits to, and the spend record keeps it), a per-request nonce, and the nullifier. It never learns an address, a voting key, or which node. It also observes network and timing metadata unless it sits behind other infrastructure.
