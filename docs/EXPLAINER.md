...
- The doorman still has to trust something about where the list came from, though less than before. Part of that trust has moved, and part of it has not.
  ...
  What remains is narrower than it was. Dash's fastest confirmation, called a ChainLock, is a signature from a group of masternodes saying a block is settled. The doorman is told a block has one and does not check that signature, so a node could offer a genuine but old block, or one mined at the lowest difficulty the network permits, which costs real effort but far less than the current chain.

  Checking the ChainLock signature sounds like a small fix, and a prototype during this work confirmed the signature itself can be checked in the doorman's own code. But it is not a shortcut, and here is the reason, because it is easy to get wrong. A signature is only meaningful if the doorman independently knows which masternodes were entitled to sign it. That list of signers is itself recorded in the block, so reading it out of the block the node just handed over is circular: a node that fabricates a cheap block can also fabricate the signer list inside it and a matching signature, and every check passes. Learning the true signer set without trusting the node is exactly the lightweight-wallet work, following the chain forward from a fixed, trusted starting point. So there is no small version of this. It is the full light-client step, and it is not built. The verification pieces it would use are written and tested, but on their own they do not close the gap. The practical summary is unchanged: a node can no longer make things up for free, and a determined node with real resources is not yet ruled out, so do not gate anything of value on this alone.
