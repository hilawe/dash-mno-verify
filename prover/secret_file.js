...
import { open, readFile, writeFile, chmod, readdir } from "node:fs/promises";
...
export async function markSecretAccepted(path, extra = {}) {
  ...
  await writeFile(path, JSON.stringify({ ...current, ...extra, status: "accepted" }, null, 2));
...
export async function findSecretForContext(contextHashDec, dir = ".") {
  const candidates = [];
  let names = [];
  try {
  ...
  const accepted = candidates.find((c) => c.rec.status === "accepted");
  return (accepted ?? candidates[0]).path;
}
...
export async function resolveSecret(path) {
