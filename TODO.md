...
- [ ] Make the adapter prove instructions mode-aware. The adapters tell members to run `npm run prove` (single-tier), which is correct for the default single-tier build but wrong for a two-tier deployment, where the member runs `npm run prove-epoch -- --challenge challenge.json`. Return `mode` from `/v1/challenge` and have each adapter render the matching command. This is a pre-existing single-vs-two-tier gap, not specific to the auth change. (`core/gateway.js`, the four adapters)
